...
import { BlockMap, MemoryBlockstore, MST, blocksToCarFile, } from "@atproto/repo";
import { pinBlob } from "@etzhayyim/ipfs";
import { decrypt as aeadDecrypt, encrypt as aeadEncrypt, generateKey, KEY_BYTES, } from "@etzhayyim/pqh/crypto";
...
const ENCRYPTED_WRAPPER_MARKER = "_etz_encrypted";
export class CheckpointerSidecar {
    ...
    constructor(cfg) {
        this.cfg = {
            ...
            blobInlineThreshold: cfg.blobInlineThreshold ?? DEFAULTS.blobInlineThreshold,
            encryptCells: cfg.encryptCells ?? new Set(),
        };
    }
    async start() {
    ...
    async #dispatch(req) {
        ...
        if (req.op === "health")
            return ok({ data: encode({ status: "ok" }) });
    ...
    async #put(req) {
        ...
        const checkpointId = req.checkpoint_id;
    ...
    async #list(req) {
        const rows = [...this.#indexCache.values()]
            .filter((r) => r.cell_did === req.cell_did &&
    ...
    async #anchorPending(req) {
        const rows = [...this.#indexCache.values()].filter((r) => r.cell_did === req.cell_did &&
            r.ipfs_pinned_at !== null &&
            r.anchor_tx_hash === null);
        return ok({ data: encode(rows) });
    }
    async #anchorCommit(req) {
        if (!req.payload)
            return err("anchor_commit requires payload");
        const commits = decode(req.payload);
        for (const c of commits) {
            const row = this.#indexCache.get(indexKey({
                cell_did: req.cell_did,
    ...
    async #commitMst(req) {
        if (!req.checkpoint_id)
            throw new Error("commitMst requires checkpoint_id");
        if (!req.payload)
            throw new Error("commitMst requires payload");
        ...
        const payload = decode(req.payload);
        ...
        mst = await mst.add(`kotodama.cell.checkpoint/${req.checkpoint_id}`, recordCid);
    ...
    // ─── Spool + index ────────────────────────────────────────────────
    async #spoolCar(row, carBytes) {
        const dir = join(this.cfg.stateDir, "queue", encodeURIComponent(row.cell_did));
        await mkdir(dir, { recursive: true });
        const path = join(dir, `${row.checkpoint_id}.car`);
        await writeFile(`${path}.tmp`, carBytes);
        await rename(`${path}.tmp`, path);
    }
    async #spoolPayload(row, payload) {
        const dir = join(this.cfg.stateDir, "queue", encodeURIComponent(row.cell_did));
        const path = join(dir, `${row.checkpoint_id}.payload`);
        await writeFile(`${path}.tmp`, payload);
        await rename(`${path}.tmp`, path);
    }
    async #loadPayload(row) {
        const path = join(this.cfg.stateDir, "queue", encodeURIComponent(row.cell_did), `${row.checkpoint_id}.payload`);
        return new Uint8Array(await readFile(path));
    }
    async #pinSoon(row, carBytes) {
        if (!this.cfg.ipfsApiUrl)
            return;
        try {
    ...
    #latestFor(cell_did, thread_id, checkpoint_ns) {
        let best;
        for (const r of this.#indexCache.values()) {
            if (r.cell_did === cell_did &&
    ...
    async #loadIndex() {
        ...
        for (const r of rows)
            this.#indexCache.set(indexKey(r), r);
        this.#indexLoaded = true;
    }
    async #persistIndex() {
        if (!this.#indexLoaded)
            return;
        const path = join(this.cfg.stateDir, "index.json");
        await writeFile(`${path}.tmp`, JSON.stringify([...this.#indexCache.values()]));
        await rename(`${path}.tmp`, path);
    }
    // ─── Encryption (per-cell symmetric key, ADR-2605181100) ──────────
    async #getOrCreateCellKey(cellDid) {
        ...
        if (existsSync(path)) {
            const k = new Uint8Array(await readFile(path));
            if (k.length !== KEY_BYTES) {
                throw new Error(`[checkpointer] cell key for ${cellDid} has wrong length: ${k.length}`);
            }
    ...
    async #encryptPayload(cellDid, plaintextBytes) {
    ...
    async #decryptPayload(cellDid, blob) {
...
function isEncryptedWrapper(v) {
    return (typeof v === "object" &&
        v !== null &&
        v[ENCRYPTED_WRAPPER_MARKER] ===
            1 &&
 ...
 */
function resolveDidList(raw, fallbackAllowed) {
    if (!raw)
        return [];
    if (raw === "*" && fallbackAllowed)
...
export async function runFromEnv() {
    ...
    if (allowed.length === 0) {
        throw new Error("ETZ_CHECKPOINTER_ALLOWED_DIDS must list at least one DID " +
            "(comma-separated, @/abs/path, or *)");
    }
    const allowedSet = new Set(allowed);
    const encryptCells = new Set(resolveDidList(process.env.ETZ_CHECKPOINTER_ENCRYPT_CELLS, allowedSet));
    for (const did of encryptCells) {
        if (!allowed.includes(did)) {
            throw new Error(`ETZ_CHECKPOINTER_ENCRYPT_CELLS lists ${did} not in ` +
